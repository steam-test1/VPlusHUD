if string.lower(RequiredScript) == "lib/managers/hud/hudobjectives" then
	...
	function HUDObjectives:init(hud)
		if VHUDPlus:getSetting({"AssaultBanner", "USE_CENTER_ASSAULT"}, true) or not VHUDPlus:getSetting({"CustomHUD", "ENABLED_ENHANCED_OBJECTIVE"}, true) then
		...
		self._panel = hud.panel:panel({
			...
			h = 130,
			w = 400,
			x = (VHUDPlus:getSetting({"TabStats", "CLOCK_MODE"}, 3) == 4) and 0 or 80,
			valign = "top"
		})
	...
	function HUDObjectives:apply_offset(offset)
		if offset and offset ~= self._offset_y then
			...
			if alive(self._bg_box) then
				...
				self._panel:animate(callback(self, self, "_animate_move"), (self._offset_y > 40 or VHUDPlus:getSetting({"TabStats", "CLOCK_MODE"}, 3) == 4) and 0 or 80, self._panel:y(), true)
			end
		end
...
elseif string.lower(RequiredScript) == "lib/managers/hud/hudheisttimer" then
	...
	function HUDHeistTimer:init(hud, tweak_hud)
		...
		self._enabled = (VHUDPlus:getSetting({"TabStats", "CLOCK_MODE"}, 3) ~= 4) and not (tweak_hud and tweak_hud.no_timer)

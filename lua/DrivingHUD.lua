if string.lower(RequiredScript) == "lib/managers/hud/huddriving" then
	...
	function HUDDriving:init(hud)
		...
		local vis = VHUDPlus:getSetting({"DrivingHUD", "SHOW_HEALTH"}, true)
	...
	function HUDDriving:start()
		self._active = true
		self._people = 0
		self.drivingpanel:set_visible(VHUDPlus:getSetting({"DrivingHUD", "ENABLED"}, true))
	end
	...
	function HUDDriving:set_vehicle_state(speed, rpm, gear, people, people_total, name, seats_table, loot_current, loot_total, health_current, health_total)
		if  VHUDPlus:getSetting({"DrivingHUD", "SPEED_IN_MPH"}, true) then 
		...
		local health_vis = VHUDPlus:getSetting({"DrivingHUD", "SHOW_HEALTH"}, true)
			...
			for i, seat in pairs(seats_table) do
				...
				if alive(seat.occupant) then
					if self.drivingpanel:child(seat.name) then
						...
						icon:set_visible(VHUDPlus:getSetting({"DrivingHUD", "SHOW_VEHICLE"}, true))
	...
	function HUDDriving:update_images()
		if main_table[self._name] then
			...
			vehicle_bitmap:set_visible(VHUDPlus:getSetting({"DrivingHUD", "SHOW_VEHICLE"}, true))
